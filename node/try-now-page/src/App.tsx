  ...
  const handleStartSession = (): void => {
    ...
    launch(
      config.useEphemeralStorage
        ? LaunchRequest.ephemeral(config.serviceUrl, config.appId, config.appDefinition, 5)
        : LaunchRequest.createWorkspace(config.serviceUrl, config.appId, config.appDefinition, 5)
    )
      .catch((_err: Error) => {
        setError(
          'Sorry, there are no more available instances in the cluster.\n' +
...
const LaunchApp: React.FC<LaunchAppProps> = ({ appName, onStartSession }) => {
  ...
  return (
    <div>
      <AppLogo />
      <p>
        <button className='App__try-now-button' disabled={!acceptedTerms} onClick={onStartSession}>
          Launch {appName} &nbsp;&nbsp;&rarr;
        </button>
      </p>
